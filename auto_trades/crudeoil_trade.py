...
Usage:
    python crudeoil_trade.py [--symbol WTICrude] [--direction BUY] [--volume 0.1] [--sl 76.50] [--tp 85.00]
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Crude Oil Auto-Execution Script")
    parser.add_argument("--symbol", type=str, default="WTICrude", help="Trading symbol (default: WTICrude)")
    ...
    # Connect to MT5
    print("=" * 70)
    print(f"CRUDE OIL AUTO-EXECUTION: {args.symbol}")
    print("=" * 70)
    print()
    ...
    # Default SL/TP if not provided (based on WTI volatility)
    if args.sl is None:
    ...
    if args.tp is None:
        TAKE_PROFIT_1 = ENTRY_PRICE + 6.5 if DIRECTION == "BUY" else ENTRY_PRICE - 6.5
        TAKE_PROFIT_2 = ENTRY_PRICE + 13.0 if DIRECTION == "BUY" else ENTRY_PRICE - 13.0
    else:
        TAKE_PROFIT_1 = args.tp
        TAKE_PROFIT_2 = args.tp + (args.tp - ENTRY_PRICE) * 0.5

    # Position sizing: 2% risk
    risk_amount = acc.balance * 0.02
    risk_points = abs(ENTRY_PRICE - STOP_LOSS)
    position_size = round(risk_amount / (risk_points * 100), 2)  # WTI: ~$100 per 0.01 lot per $1
    ...
    print(f"Risk/Reward: 1:{abs(blended_tp-ENTRY_PRICE)/risk_points:.1f}")
    print()
